 ...
 * `session-ended.md §5 Processing Rules` rule 2. This simulator read the
 * wall clock at both, which is what this module exists to stop.
 *
 * `heartbeat.md §6 Clock Synchronization` rule 6 states the division of labour, and it is the whole of
 ...
 *
 * Why the rule is a MUST and not advice, in `heartbeat.md §6 Clock Synchronization` rule 6's own terms:
 ...
 * restart cannot be measured this way at all (`heartbeat.md §6 Clock Synchronization` says as much for
